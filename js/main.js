...
{ // Scene Progression (Ordered)
    ...
    function next_scene(scene, fromload) {
        ...
        if (player.config.devmode.scene_tracking) console.log(`Attempting to update previous scene from ${player.previous_scene} to ${player.scene}`);
...
{ // Scene Checks
    ...
    function validate_option_scene(option) {
        ...
        if (story[story_map.indexOf(option.scene)]?.options?.length === undefined) return true;
        else return false;
    }
}
