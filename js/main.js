...
function option_progress_scene(option) {
  ...
  if (option?.time !== undefined) {
    ...
    if (player.config.debug > 3)
      console.log('Option time tag absent, doing default time incrementation.');
  }
...
function display_scene(scene, fromload) {
  ...
  for (let i = 0, len = scene.text.length; i < len; i++) {
    if (player.config.debug > 1)
      console.log(`%cChecking text ${i + 1} conditions.`, 'color: aquamarine');
...
function display_options(scene) {
  ...
  for (let i = 0, len = scene.options.length; i < len; i++) {
    if (player.config.debug > 1)
      console.log(`%cChecking option ${i + 1} conditions.`, 'color: turquoise');
  ...
  for (let i = 0, len = valid_option.length; i < len; i++) {
    ...
    if (valid_option_true[i] == true) {
      ...
      if (player.config.keybinds == true)
        btn.innerHTML =
          keybind_text(current_options_displayed.length) + valid_option[i].text;
      ...
      if (
        ...
        btn.innerHTML = btn.innerHTML + get_timestamps(valid_option[i].time);
  ...
  if (part.norepeat !== undefined) {
    ...
    if (typeof part.norepeat === 'boolean') {
      if (part.norepeat == false) {
        ...
        if (
          player.history[player.history.map((e) => e.id).indexOf(player.scene)]
            ?.amount > 1
        ) {
          if (player.config.debug > 3)
            console.log(
      ...
      if (
        player.history[player.history.map((e) => e.id).indexOf(part.norepeat)]
          ?.amount > 1
      ) {
        if (player.config.debug > 3)
          console.log('Repeat condition failed, not visit on OTHER scene.');
        return false;
      }
      ...
      if (part.norepeat.length == 2) {
        ...
        let visit_amount =
            player.history[
              player.history.map((e) => e.id).indexOf(part.norepeat[0])
            ]?.amount,
          min = part.norepeat[1],
          max = part.norepeat[2];
        if (min === true) {
    ...
    if (player_stats.length != local_tracked_stats.length) {
      ...
      for (let i = 0, len = player_stats.length; i < len; i++) {
        ...
        p.innerHTML = `${get_sign(signed_delta)}${delta} ${
          player_stats[i].charAt(0).toUpperCase() + player_stats[i].slice(1)
        }`;
        output_array.push(p);
      }
    }
...
function start_trade(vendor_name, modifiers, global_modifiers) {
  ...
  for (let i = 0, len = trade_items.length; i < len; i++) {
    ...
    if (inventory_map.indexOf(trade_items[i].name) == -1)
      ...
      p3.innerHTML =
        'Own: ' +
        player.inventory[inventory_map.indexOf(trade_items[i].name)].count;
    d3.appendChild(p3);
    // cost
    ...
    if (player.gold < cost)
      p2.innerHTML = "<span style='color: lightcoral'>" + cost + ' Gold</span>';
...
function update_item_gold_color() {
  ...
  for (let i = 0, len = items.length, g = player.gold; i < len; i++) {
    ...
    if (g < items[i].innerHTML.slice(0, items[i].innerHTML.indexOf(' ')))
      items[i].innerHTML =
        "<span style='color: lightcoral'>" + items[i].innerHTML + '</span>';
  }
}
