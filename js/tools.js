...
function what_points_to(scene_id) {
  ...
  console.log(
    `${found_scenes.length} ${str} points to scene ${scene_id} (${
      Date.now() - start
    }ms)`,
    found_scenes
  );
...
function meta_check(min, max) {
  ...
  for (let i = 0; i < len; i++, checked++) {
    if (story[story_map.indexOf(id_array[i])]?.meta === undefined)
      no_meta.push(i);
  }
  console.log(
    `${no_meta.length}/${checked} scenes are missing metadata (${
      Date.now() - start
    }ms)`,
    no_meta
  );
...
function ignominy_export(mode, explicit) {
  // exports combined story JSON
  // explicit currently unused
  if (mode == 'story') {
    let file = new Blob([JSON.stringify(story, null, 2)], {
